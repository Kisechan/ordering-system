...
QJsonObject AuthService::login(const QString& username, const QString& password) const {
    if (!isAlphaNumOnly(password)) {
        return makeErr(402, "密码只能包含字母和数字！");
    }
    ...
    if (dv.isNull() || !dv.isObject()) {
        return makeErr(402, "该用户不存在！");
    }

    const auto u = dv.toObject();
    if (u.value("password").toString() != password) {
        return makeErr(401, "密码错误！");
    }
...
QJsonObject AuthService::registerUser(const QString& username, const QString& password) const {
    ...
    if (found.value("code").toInt() != 200) {
        return found;
    }

    if (!found.value("data").isNull()) {
        return makeErr(403, "用户名已存在！");
    }
